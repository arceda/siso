...
def train(batch_size=4, nb_epoch=30):

    checkpointer = ModelCheckpoint(
        filepath=os.path.join('data_10', 'checkpoints', 'lstm' + '-' + 'features' +
                              '.{epoch:03d}-{val_loss:.3f}.hdf5'),
        verbose=1,
        save_best_only=True)

    tb = TensorBoard(log_dir=os.path.join('data_10', 'logs', 'lstm'))

    early_stopper = EarlyStopping(patience=30)

    timestamp = time.time()

    csv_logger = CSVLogger(os.path.join('data_10', 'logs', 'lstm' + '-' + 'training-' +
                                        str(timestamp) + '.log'))
    ...
    # del rm.model  # deletes the existing model
